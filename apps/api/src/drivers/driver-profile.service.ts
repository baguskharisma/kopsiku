  ...
  export class DriverProfileService {
    ...
    async createDriverProfile(
      ...
      if (user.role !== Role.DRIVER) {
        throw new ForbiddenException('Hanya user dengan role DRIVER yang dapat membuat driver profile');
      }
      ...
      const driverProfile = await this.prisma.driverProfile.create({
        data: {
          ...
          qrImageUploadedAt: createDriverProfileDto.qrImageUrl ? new Date() : null,
          qrImageVerified: false, // Always false when created
          maxRadius: createDriverProfileDto.maxRadius || 10,
          preferredVehicleTypes: createDriverProfileDto.preferredVehicleTypes || [],
    ...
    async updateDriverProfile(
      ...
      if (updateDriverProfileDto.licenseNumber && 
        ...
        if (existingLicense && existingLicense.id !== driverProfile.id) {
          throw new ConflictException('Nomor SIM sudah terdaftar oleh driver lain');
        }
      }
  
      // Check ID card uniqueness if being updated
      if (updateDriverProfileDto.idCardNumber && 
        ...
        if (existingIdCard && existingIdCard.id !== driverProfile.id) {
          throw new ConflictException('Nomor KTP sudah terdaftar oleh driver lain');
        }
      }
      ...
      if (updateDriverProfileDto.qrImageUrl && 
    ...
    async getDriverProfileById(profileId: string): Promise<DriverProfileResponseDto> {
      const driverProfile = await this.prisma.driverProfile.findUnique({
        where: { id: profileId },
      });
    ...
    async getAllDriverProfiles(options: {
      ...
      const { page = 1, limit = 10, status, isVerified, qrImageVerified, search } = options;
