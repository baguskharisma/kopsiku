import {
  ...
  export class DriverProfileController {
    ...
    @ApiOperation({ 
      ...
      description: 'Create a new driver profile. Only users with DRIVER role can create driver profiles.'
    })
    @ApiResponse({ 
      status: 201, 
    ...
    @ApiResponse({ status: 409, description: 'Conflict - profile already exists or duplicate data' })
    async createProfile(
      @GetUserId() userId: string,
      @Body() createDriverProfileDto: CreateDriverProfileDto,
    ): Promise<DriverProfileResponseDto> {
      return this.driverProfileService.createDriverProfile(userId, createDriverProfileDto);
    }
    ...
    @ApiOperation({ 
      ...
      description: 'Get the current driver\'s profile information including QR image status.'
    })
    @ApiResponse({ 
      status: 200, 
    ...
    async getMyProfile(@GetUserId() userId: string): Promise<DriverProfileResponseDto> {
    ...
    @ApiOperation({ 
      ...
      description: 'Update the current driver\'s profile. Updating QR image will reset verification status.'
    })
    @ApiResponse({ 
      status: 200, 
    ...
    async updateMyProfile(
      ...
      return this.driverProfileService.updateDriverProfile(userId, updateDriverProfileDto);
    }
    ...
    @ApiOperation({ 
      ...
      description: 'Update or upload QR code image for bank account. This will reset verification status.'
    })
    @ApiResponse({ 
      status: 200, 
    ...
    async updateQrImage(
      ...
      return this.driverProfileService.updateDriverQrImage(userId, updateQrImageDto);
    }
    ...
    @ApiOperation({ 
      ...
      description: 'Get paginated list of all driver profiles with filtering options.'
    })
    @ApiQuery({ name: 'page', required: false, type: Number, description: 'Page number (default: 1)' })
    @ApiQuery({ name: 'limit', required: false, type: Number, description: 'Items per page (default: 10)' })
    @ApiQuery({ name: 'status', required: false, enum: DriverStatus, description: 'Filter by driver status' })
    @ApiQuery({ name: 'isVerified', required: false, type: Boolean, description: 'Filter by verification status' })
    @ApiQuery({ name: 'qrImageVerified', required: false, type: Boolean, description: 'Filter by QR image verification status' })
    @ApiQuery({ name: 'search', required: false, type: String, description: 'Search by license number, ID card, address, bank account, or user info' })
    @ApiResponse({ 
      status: 200, 
    ...
    async getAllProfiles(
      ...
      const options = {
        ...
        qrImageVerified: qrImageVerified !== undefined ? qrImageVerified === 'true' : undefined,
        search,
      };
    ...
    @ApiOperation({ 
      ...
      description: 'Get list of drivers who have uploaded QR images but are not yet verified.'
    })
    @ApiResponse({ 
      status: 200, 
    ...
    @ApiResponse({ status: 400, description: 'Bad request - QR image not uploaded' })
    @ApiResponse({ status: 404, description: 'Driver profile not found' })
    async verifyQrImage(
      @Param('id', ParseUUIDPipe) profileId: string,
    ...
    @ApiOperation({ 
      ...
      description: 'Verify driver profile and update status. This is separate from QR image verification.'
    })
    @ApiParam({ name: 'id', description: 'Driver profile ID' })
    @ApiResponse({ 
      status: 200, 
    ...
    @ApiOperation({ 
      ...
      description: 'Get statistics about QR image verification status across all drivers.'
    })
    @ApiResponse({ 
      status: 200, 
