...
export class OtpService {
  ...
  constructor(
    ...
    this.resendCooldownSec = this.config.get<number>(
  ...
  async sendOtp(
    phone: string,
    purpose: 'register' | 'login' | 'reset' | 'verify_phone',
    userId?: string,
  ) {
    ...
    if (latest) {
      ...
      if (diffSec < this.resendCooldownSec) {
        throw new BadRequestException(
          `Please wait ${Math.ceil(this.resendCooldownSec - diffSec)}s before requesting another OTP`,
        );
      }
    }
